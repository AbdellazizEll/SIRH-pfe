...
type motifName = 'PAID_LEAVE' | 'UNPAID_LEAVE' | 'SICK_LEAVE' | 'MATERNITY_LEAVE' | 'PATERNITY_LEAVE' | 'PARENTAL_LEAVE' | 'COMPASSIONATE_LEAVE' | 'STUDY_LEAVE' | 'SABBATICAL_LEAVE' | 'MARRIAGE_LEAVE' | 'ADOPTION_LEAVE' | 'PUBLIC_HOLIDAYS' | 'MEDICAL_LEAVE' | 'BEREAVEMENT_LEAVE' | 'EMERGENCY_LEAVE';
...
export class GestionAbsenceComponent implements OnInit {
  ...
  loadTabData(): void {
    ...
    this.absenceState$ = this.absenceService.getAbsencesManagerWithFilters(
      user.id,
  ...
  approveAbsence(demandeId: number): void {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        ...
        this.absenceService.approveAbsenceRequestManager(demandeId).subscribe(
          response => {
  ...
  // Method to open the rejection dialog
